...
sd_init()
{
    ...
    cprintf("- sd init: LBA of 1st block 0x%x, 0x%x blocks totally\n", first_bno, nblocks);
}
...
sdInit()
{
    ...
    if (resp == SD_OK) {
        ...
        // delay(50);
        // Resolve voltage.
        if ((resp = sdAppSendOpCond(ACMD41_ARG_SC))) return sd_debug_resp(resp);
